...
export function useRiderStreamConnection(location: Coordinate, riderID: string) {
  ...
  useEffect(() => {
    ...
    const ws = new WebSocket(`${WEBSOCKET_URL}${BackendEndpoints.WS_RIDERS}?riderID=${riderID}`);
